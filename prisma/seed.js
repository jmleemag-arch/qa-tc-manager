            ...
            ? {
                create: selectedCases.map((testCase, index) => {
                  ...
                  return {
                    ...
                    result,
                    executedAt: result ? createdAt : null,
                  };
                }),
              }
            : undefined,
...
async function seedUserTasks() {
  ...
  const tester = await prisma.user.findUnique({ where: { userId: "tester1" } });
  const jumi = await prisma.user.findUnique({ where: { userId: "lee-jumi" } });
